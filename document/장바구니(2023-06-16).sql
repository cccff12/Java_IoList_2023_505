...
-- 판매내역 테이블에서 사용할 SEQUENCE 객체 생성하기
-- 일련번호를 자동으로 생성하기 위한 객체

-- 1부터 1씩 자동으로 증가하는 일련번호를 만들겠다.
...
/*
tbl_buyer list 에 데이터를 추가 할때 
ioSEQ 칼럼에 자동 증가된 값을 추가하기 위하여 항상 seq_iolist.NEXTVAL이라는 코드를 입력해야 한다.
간혹 실수로 코드를 생략하면 INSERT 명령에서 오류가 발생한다.
...
--DEFAULT 속성은 데이터가 실행될때 자동으로 입력되는 코드
CREATE TABLE tbl_iolist(
ioSEQ	NUMBER DEFAULT seq_iolist.NEXTVAL PRIMARY KEY,
ioDate	VARCHAR2(10)	NOT NULL,	
ioTime	VARCHAR2(10)	NOT NULL,	
