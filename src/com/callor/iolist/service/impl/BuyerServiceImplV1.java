...
//	BuyerDao buyerDao;
//	객체의 초기화: 선언된 객체를 통해 변수, method에 접근할 수 있도록
//	하는 조치
//	BuyerDao buyerDao = new BuyerDao(); => 생성자로 객체 초기화
//	객체의 생성 : 선언되고 초기화까지 완성된 객체, 사용할 준비가 된 상태
public class BuyerServiceImplV1 implements BuyerService {
//	Service에서 Dao의 method를 사용하기 위한 객체 선언
//	클래스 영역에 선언된 객체는 final 키워드를 필수적으로 사용하자
//	final 키워드를 사용해 선언된 객체는 만약 초기화가 되지 않으면 문법 오류를 보여 반드시
//	초기화 하도록 독려한다.
//	final로 선언된 객체는 반드시 현재 클래스의 생성자에서 초기화를 해 주어야 한다.
...
//		new 생성자를 사용하지 않고
//		SqlSession getMappter()method를 통해 
//		BuyerDao 객체를 전달받아 초기화 한다.
	...
	public void insert() {
		...
		while (true) {
			...
			System.out.println("추가할 고객 정보를 확인");
			System.out.println("고객 id를 입력하세요. QUIT: 종료, NEW:ID 생성");
...
//			고객 ID가 strBuId에 담겨있는데
//			키보드로 입력한 ID이거나, 새로 생성한 ID일 것이다.
//			strBuId 에 담긴 고객 ID를 findByBuId()에 전달한다.
			buyerDto = this.findByBuId(strBuId);
//			조회된 고객정보가 null이면
//			새로운 고객정보를 저장할 Dto를 만들고
//			고객 ID를 setting
			if (buyerDto == null) {
