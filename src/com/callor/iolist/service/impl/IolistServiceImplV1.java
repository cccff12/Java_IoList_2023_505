...
//			날자형 데이터를 문자열형 데이터로 변환
//			2023-06-21형식의 문자열 생성
			String curDate = checkDate.format(date);
//			날자 형식을 갖춘 문자열형 데이터를 날짜형 (date type) 의 데이터로 변환
//			"2023-06-21">> 2023-06-21 날짜 type 데이터
//			이 method는 simpledateformatter 에 설정된 형식 문자열(YYYY-MM-dd)에 일치하지 않으면
//			Exception 을 발생시킨다.
...
//				날짜type-> 문자열로
	...
	/*
	 * 기간별 거래 리스트 조회시작일자, 조회종료일자를 입력받고 해당기간의 리스트를 출력하기
	 */
	@Override
	public void selectBetweenDate() {
		while (true) {
			System.out.println("기간별 이스트를 출력하기 위하여 날짜를 입력");
			System.out.println("조회시작 일자를 입력하세요");
			String sDate = this.inputDate();
			if (sDate == null)
				return;

			System.out.println("조회종료 일자를 입력하세요");
	...
	public void selectByBuyer() {
		...
		if (buyerDto == null) {
			HelpMessage.ERROR("고객정보가 없어요 ㅠㅠ");
		} else {
	...
	public void selectByProduct() {
		...
		if (productDto == null) {
			HelpMessage.ERROR("상품정보는 찾을 수 없습니다.");
		} else {
	...
	public void selectByBuyerBetweenDate() {
		...
		while (true) {
			...
			System.out.println("조회 시작일자>>");
			String sDate = inputDate();
			if (sDate == null)
				return;

			System.out.println("조회 종료일자>>");
